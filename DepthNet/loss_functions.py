...
def compute_errors_train(gt, pred, valid):
    ...
    for current_gt, current_pred, current_valid in zip(gt, pred, valid):
        ...
        if len(valid_gt) == 0:
            ...
            a2 += (thresh < 1.25 ** 2).float().mean() #what's this
            a3 += (thresh < 1.25 ** 3).float().mean() #what's this

            abs_diff += torch.mean(torch.abs(valid_gt - valid_pred))
            abs_rel += torch.mean(torch.abs(valid_gt - valid_pred) / valid_gt)

            sq_rel += torch.mean(((valid_gt - valid_pred) ** 2) / valid_gt)  # What's this?
